...
//           <div
//             key={project.id}
//             className="relative rounded-3xl overflow-hidden transition-transform duration-500 hover:-translate-y-4 hover:scale-105 border border-gray-200 bg-white"
//           >
...
//                 <div
//                 //  className="absolute inset-0 bg-black/20 flex flex-col justify-end p-6 opacity-0 hover:opacity-100 transition-opacity duration-500"
//                  >
...
const CssProjects = () => {
  return (
    <section className="py-16 px-4 max-w-[1280px] mx-auto bg-gray-50">
      ...
      <p className="text-center text-gray-500 mb-14 max-w-4xl mx-auto text-base md:text-lg">
        This section showcases my CSS-based projects where I focused on creating 
      visually appealing and responsive layouts. These projects highlight my skills in styling with
       Flexbox, Grid, media queries, transitions, and animations. Each project is designed to 
       improve user experience and aesthetics, with a live preview and GitHub link available for 
       every build.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {cssProjects.map((project, index) => (
          <div
            key={index}
            className="relative rounded-3xl overflow-hidden border border-gray-200 bg-white transition-transform duration-500 hover:-translate-y-4 hover:scale-105"
          >
            {/* Gradient glow */}
            <div className="absolute -inset-0.5 bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 blur opacity-40 rounded-3xl"></div>
